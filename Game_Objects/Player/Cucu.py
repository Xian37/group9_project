...
class Cucu(Player):
    def __init__(self, x, y, size, color, speed, role3_cucu):
        super().__init__(x, y, size, color, speed,
                         health=100, player_path=role3_cucu, name='You')
    ...
    def draw_health_block(self, screen):
        ...
        fill_rect = pygame.Rect(block_x, block_y + bar_height-(self.health / 100)
                                * bar_height, bar_width, (self.health / 100) * bar_height)
        rainbow_image = pygame.transform.scale(
            rainbow_image, (80, 525*(self.health / 100)))
