...
class Bomb(GameObject):
    ...
    def explode(self, map_data, flames, player, enemies):
        bomb_x, bomb_y = (self.x - MARGIN_WIDTH) // TILE_SIZE, (self.y - 10) // TILE_SIZE
        explosion_range = 1
            ...
            for step in range(1, explosion_range + 1):
                ...
                if 0 <= check_x < len(map_data[0]) and 0 <= check_y < len(map_data):
                    if map_data[int(check_y)][int(check_x)] == 1:
                        ...
                        flames.append(Flame(check_x * TILE_SIZE + MARGIN_WIDTH, check_y * TILE_SIZE + 10, TILE_SIZE, ORANGE, time.time()))
                        break
